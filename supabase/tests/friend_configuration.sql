...
declare a uuid:=gen_random_uuid(); b uuid:=gen_random_uuid(); c uuid:=gen_random_uuid();
 ...
 nc text:='PresC'||left(replace(c::text,'-',''),12); result jsonb; started timestamptz; code text; spawn text;
begin
 ...
 if result?'presence' or result?'components' or result?'avatar_revision' or result?'configuration' then raise exception 'pending request leaks detail';end if;
 ...
 foreach spawn in array array['1','2','4'] loop
  reset role;
  update paw_private.social_presence set seen_at=now()-interval '5 seconds' where player_id=a;
  perform set_config('request.jwt.claims',jsonb_build_object('sub',a,'session_id',sa,'role','authenticated')::text,true);
  set local role authenticated;
  code:='PAW-BETA-IW0-SP'||spawn||'-RM1-SG0-LM1-RU1-CL1-OOS1-PS0';
  if public.paw_presence(true,'beta','{"core":true}',code)->>'status'<>'ok' then raise exception 'exact write';end if;
  if public.paw_presence(true,'stable','{}',code)->>'status'<>'invalid_presence' then raise exception 'mismatched code channel';end if;
  if public.paw_presence(true,'beta','{}',code||'-PATH-private')->>'status'<>'invalid_presence' then raise exception 'malformed code';end if;
  if public.paw_presence(true,'beta','{}',repeat('x',129))->>'status'<>'invalid_presence' then raise exception 'oversized code';end if;
  if public.paw_presence(true,'beta','{}',replace(code,'SP'||spawn,'SP3'))->>'status'<>'invalid_presence' then raise exception 'unknown spawn';end if;
  perform set_config('request.jwt.claims',jsonb_build_object('sub',b,'session_id',sb,'role','authenticated')::text,true);
  result:=public.paw_social_list()->'players'->0;
  if result->>'configuration'<>code or result->'components'->>'powers_shards'<>'false'
   or result->'components'->>'roaming'<>(case when spawn='1' then 'false' else 'true' end)
   or result->'components'->>'hostility'<>'false' then raise exception 'exact roundtrip mismatch'; end if;
 end loop;
 reset role;
 ...
 reset role;
 if (select configuration from paw_private.social_presence where player_id=a) is not null then raise exception 'legacy client retained stale configuration';end if;
 ...
 if public.paw_presence(false,'stable','{}','PAW-STABLE-IW0-SP1-RM0-SG0-LM1-RU0-CL0-OOS0')->>'status'<>'session_expired' then raise exception 'stale instance wrote exact configuration';end if;
 reset role;
 ...
 result:=public.paw_social_list()->'players'->0;
 if result?'presence' or result?'components' or result?'avatar_revision' or result?'configuration' then raise exception 'block leaks detail';end if;
 reset role;
 ...
 raise notice 'FRIEND CONFIGURATION PASS: exact SP1/SP2/SP4 and PS0 roundtrip, malformed/mismatch/oversize, legacy clear, friend-only metadata/avatar, pending/stranger/block denial, no direct access, TTL, duration, game exit, takeover and bounded input';
end; $$;
rollback;
