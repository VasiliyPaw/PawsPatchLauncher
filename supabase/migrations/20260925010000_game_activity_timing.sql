-- Explicit native pause and simulation speed; older publishers may omit both.
...
begin
 ...
 or exists(select 1 from jsonb_object_keys(a) k where k not in ('phase','multiplayer','elapsed','width','height','room','self','players','paused','speed'))
 ...
 if a->'paused' is not null and a->'paused'<>'null'::jsonb and jsonb_typeof(a->'paused')<>'boolean' then return false; end if;
 ...
 for p in select value from jsonb_array_elements(people) loop
  if jsonb_typeof(p)<>'object' or exists(select 1 from jsonb_object_keys(p) k where k not in ('key','name','bot','profile','team','color','race','subrace','observer'))
  ...
  if p->'observer' is not null and p->'observer'<>'null'::jsonb and jsonb_typeof(p->'observer')<>'boolean' then return false; end if;
