...
internal static class SocialChecks
{
    ...
    internal static void Run(string language)
    {
        ...
        try {
            ...
            Check(Control<TextBlock>("FriendsRequestsBadgeText").Text=="1","tab counts");
            ...
            Check(Control<StackPanel>("FriendsDialogRows").Children[0] is Border,"request is not a compact card");
            Invoke(w,"SwitchSocialRequests","outgoing");
            Check(Control<StackPanel>("FriendsDialogRows").Children.Count==0 && Control<TextBlock>("FriendsDialogEmpty").Visibility==Visibility.Visible,"outgoing list leaks incoming requests");
