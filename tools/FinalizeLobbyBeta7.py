"""Verify public Beta 7 artifacts and stage canonical beta feeds; never uploads."""
...
from PrepareLobbyBeta7 import TAG, VERSION, VERSIONS, GUIDE
...
def fetch(url):
    request=urllib.request.Request(url,headers={'User-Agent':'PawsPatchLobbyBetaAudit','Cache-Control':'no-cache'})
...
def main(args):
    ...
    assert json.loads(fetch(API+'/releases/latest'))['tag_name']=='v0.8.0'
    ...
    for schema in base.SCHEMAS:
        for channel in base.CHANNELS:
            ...
            assert normalized(fetch(RAW+rel+'?lobby-beta7=baseline'))==normalized(before),'Public feed moved'
            ...
            assert {k:v for k,v in after.items() if k not in ('publishedAt','packages','changelog','patchGuide')}=={k:v for k,v in old.items() if k not in ('publishedAt','packages','changelog','patchGuide')}
            ...
            expected_guide=copy.deepcopy(old['patchGuide']);expected_guide['version']=VERSION;expected_guide['entries'].insert(0,GUIDE)
            assert after['patchGuide']==expected_guide
    assert base.sha(base.REPO/'feed/patch-guide-beta.json')==prepared['guideBefore']
    assert base.read(out/'patch-guide-beta.json')==base.read(out/'feeds/v2/beta.production.payload.json')['patchGuide']
    ...
    if args.promote:
        for schema in base.SCHEMAS:
            ...
            backup=base.REPO/'feed/history'/('beta-before-lobby-beta7-'+schema+'.json')
        ...
        shutil.copyfile(out/'patch-guide-beta.json',base.REPO/'feed/patch-guide-beta.json')
    ...
    print('BETA 7 PUBLIC VERIFIED'+('; canonical beta feeds promoted locally, push/readback still required' if args.promote else ''))
