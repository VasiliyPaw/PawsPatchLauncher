"""Verify public Beta 6 artifacts and stage canonical beta feeds; never uploads."""
...
from PrepareProphetBeta6 import TAG, VERSION, VERSIONS
...
def fetch(url):
    request=urllib.request.Request(url,headers={'User-Agent':'PawsPatchProphetBetaAudit','Cache-Control':'no-cache'})
...
def main(args):
    ...
    for schema in base.SCHEMAS:
        for channel in base.CHANNELS:
            ...
            assert normalized(fetch(RAW+rel+'?prophet-beta6=baseline'))==normalized(before),'Public feed moved'
    ...
    if args.promote:
        for schema in base.SCHEMAS:
            ...
            backup=base.REPO/'feed/history'/('beta-before-prophet-beta6-'+schema+'.json')
    ...
    print('BETA 6 PUBLIC VERIFIED'+('; canonical beta feeds promoted locally, push/readback still required' if args.promote else ''))
