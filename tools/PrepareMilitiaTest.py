...
def main():
    ...
    for package in feed['packages']:
        ...
        version = VERSION if ident != 'common-ui' else '1.3.72-ui.7-beta.8-test.7'
        if ident.startswith('localization-'):
            version = package['version']+'-city-test.7'
    ...
    feed['changelog'].insert(0, dict(category='patch', mod='arcane-wars', channel='beta', version=VERSION,
        ...
        body={'ru':'Исправлено накопление золота для приоритетных построек: дешёвые второстепенные действия больше не откладывают достижение целей по ресурсам. Приоритет пересчитывается с учётом очереди; правило выгодных рынков и лучшей золотой ветки сохранено.',
              'en':'Fixed saving for priority buildings: cheaper lower-priority actions no longer delay resource targets. Priorities are recalculated using queued work; profitable markets and their best gold-increasing branch remain respected.'}))
