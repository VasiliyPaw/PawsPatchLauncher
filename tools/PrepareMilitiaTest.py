...
def main():
    ...
    for package in feed['packages']:
        ...
        version = VERSION if ident != 'common-ui' else '1.3.72-ui.7-beta.8-test.8'
        if ident.startswith('localization-'):
            version = package['version']+'-city-test.8'
    ...
    feed['changelog'].insert(0, dict(category='patch', mod='arcane-wars', channel='beta', version=VERSION,
        ...
        body={'ru':'Исправлено автооткрытие ополчения захваченных городов: временная блокировка приказа больше не исчерпывает попытки. Стартовые города, новые города и здания учитывают галочку; название и подсказка уточнены. Накопление на приоритетные постройки сохранено.',
              'en':'Fixed automatic militia opening in captured cities: temporary command blocks no longer exhaust retries. Starting cities, new cities and buildings respect the option; its label and help are clearer. Saving for priority buildings remains intact.'}))
