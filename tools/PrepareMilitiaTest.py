...
def main():
    ...
    for package in feed['packages']:
        ...
        version = VERSION if ident != 'common-ui' else '1.3.72-ui.7-beta.8-test.6'
        if ident.startswith('localization-'):
            version = package['version']+'-city-test.6'
    ...
    feed['changelog'].insert(0, dict(category='patch', mod='arcane-wars', channel='beta', version=VERSION,
        ...
        body={'ru':'Восстановлен приоритет целей по ресурсам перед обычными золотыми постройками. На этом этапе с ресурсной постройкой сравнивается только прибыльный рынок с учётом расходов на дефицит и всей очереди. Правило лучшей золотой ветки рынков сохранено.',
              'en':'Resource targets again precede ordinary gold buildings. Only profitable markets compete with resource buildings at this stage, accounting for shortage costs and the full queue. Markets still use only their best gold-increasing branch.'}))
