...
let activityBefore,participantBefore,configurationBefore,observerBefore,timingBefore;
const activitySchema=async()=> (await db.query(`select p.oid::regprocedure::text as function,
 ...
 to_regprocedure('public.paw_presence(boolean,text,jsonb,text)'),
...
for(const file of (await readdir(base)).filter(n=>n.endsWith('.sql')&&!n.includes('scheduler')&&!n.includes('founder_grant')).sort()){
 if(process.argv.includes('--timing-proof')&&file==='20260925010000_game_activity_timing.sql')timingBefore=await activitySchema();
 ...
 try{
  const input=file==='20260919000000_social_configuration_options.sql'&&process.argv.includes('--deployment-sql')
   ...
   ? process.argv[process.argv.indexOf('--observer-deployment-sql')+1]
   : file==='20260925010000_game_activity_timing.sql'&&process.argv.includes('--timing-deployment-sql')
   ? process.argv[process.argv.indexOf('--timing-deployment-sql')+1] : new URL(file,base);
...
if(process.argv.includes('--timing-proof')){console.log(JSON.stringify({before:timingBefore,after:await activitySchema()},null,2));await db.close();process.exit(0);}
