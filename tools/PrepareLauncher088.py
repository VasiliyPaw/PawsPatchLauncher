"""Verify public launcher 0.8.8 and sign its four launcher-only catalogs.
...
def stage(a):
    ...
    entry = read(REPO / 'docs/release-0.8.8.json')
    ...
    for name, path in FEEDS.items():
        ...
        assert normalize(fetch(RAW + path + '?release088=stage')) == normalize(raw), 'Public feed moved: ' + path
        before = verify(read(REPO / path), key())
        assert before['launcher']['version'] == '0.8.7'
    ...
    write(out / 'plan.json', dict(sourceCommit=a.source_commit, baseline=baseline, notesSha256=sha(REPO / 'docs/release-0.8.8.json'), launcher=launcher))
...
def promote(a):
    ...
    assert sha(REPO / 'docs/release-0.8.8.json') == plan['notesSha256']
    for path, digest in plan['baseline'].items():
        assert sha(REPO / path) == digest, 'Local baseline moved: ' + path
        assert normalize(fetch(RAW + path + '?release088=promote')) == normalize((REPO / path).read_bytes()), 'Public baseline moved: ' + path
    for name, path in FEEDS.items():
...
def readback(a):
    ...
    print('PUBLICATION PASS: v0.8.8; 4 signed catalogs; history; game data unchanged; catalog commit ' + head)
