"""Stage, verify and promote Arcane 0.3.1-beta.2. Upload and Git push are separate steps."""
...
VERSION='0.3.1-beta.2';TAG='patch-'+VERSION
VERSIONS={'pawpatch-core':VERSION,'common-ui':'1.3.72-ui.9-beta.2','player-colors':VERSION,'desync-continue':'1.3.72-r5-beta.2'}
...
def prepare(a):
    ...
    for n,p in FEEDS.items():
        assert normalize(fetch(RAW+p+'?arcane031beta2=baseline'))==normalize((REPO/p).read_bytes())
...
def promote(a):
    ...
    assert read(out.parent/'colors-native/participant-tests.json')['passed']
    assert read(out.parent/'colors-native/popup-width-tests.json')['passed']
    assert sha(REPO/'docs'/('release-'+TAG+'.md'))==prep['notesSha256']
    for n,p in FEEDS.items():
        assert sha(REPO/p)==prep['before'][n]
        assert normalize(fetch(RAW+p+'?arcane031beta2=promotion'))==normalize((REPO/p).read_bytes())
    ...
    target=REPO/'feed/history/beta-before-arcane-031-beta2-v2.json';assert not target.exists()
