"""Finalize launcher 0.7.12 over prepared patch channels, without publishing.
...
RELEASES = {
    'patch-0.2.1': (False, {'pawpatch-core':'0.2.1','player-colors':'0.2.1','common-ui':'1.3.72-ui.5','aw-roaming-x4-new':'0.82.1.8-clean.3'}),
    'patch-0.3.0-beta.4': (True, {'pawpatch-core':'0.3.0-beta.4','player-colors':'0.3.0-beta.4','common-ui':'1.3.72-ui.5-beta.4'})}
...
def finalized_feed(source, launcher, title, body, date):
    require(source['launcher']['version'] == '0.7.11', 'Candidate already has another launcher generation')
    require(not any(e.get('category') == 'launcher' and e['version'] == VERSION for e in source['changelog']), 'Launcher history already contains 0.7.12')
...
class Finalization:
    ...
    def validate_candidate(self):
        for key, relative in FEEDS.items():
            ...
            require(feed['channel'] == channel and feed['launcher']['version'] == '0.7.11', 'Unexpected candidate channel/launcher')
        ...
        require(len(releases) == 2 and set(releases) == set(RELEASES), 'Unexpected patch releases')
    ...
    def validate_launcher(self):
        ...
        require('<Version>' + VERSION + '</Version>' in project, 'Source commit is not launcher 0.7.12')
        ...
        require(len(files) == len(self.artifact['files']) and set(files) == {'PawsPatchLauncher.exe', 'PawsPatchLauncher-v0.7.12-win-x64.zip', 'launcher.config.json'}, 'Unexpected CI distribution contents')
...
'''
        ...
        self.launcher_assets = [files['PawsPatchLauncher.exe'], files['PawsPatchLauncher-v0.7.12-win-x64.zip'],
    ...
    def stage(self):
        ...
        self.report = dict(version=VERSION, sourceCommit=self.commit, published=False, promoted=False,
                           ...
                           rollbackDirectory=str(REPO / 'feed/history' / ('before-launcher-0.7.12-' + self.commit[:12])),
    ...
    def promote(self):
        ...
        try:
            for relative, data in self.targets.items():
                ...
                temp = target.with_name('.' + target.name + '.finalize-0712-' + token + '.tmp')
