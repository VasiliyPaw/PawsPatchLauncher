...
public static class Arcane031Beta1Tests
{
    ...
    public static async Task RunAsync(string repoRoot,string stageRoot,string gameRoot)
    {
        ...
        foreach(var language in new[]{"en","ru","de","fr","cs","uk"})foreach(bool data in new[]{false,true})foreach(bool enabled in new[]{false,true})for(int mask=0;mask<8;mask++) {
            ...
            Require(packages.Any(p=>p.Id=="common-ui"&&p.Version=="1.3.72-ui.9-beta.1")==(!data&&enabled),"Incorrect native/static HUD scope");
        ...
        foreach(var language in new[]{"en","ru","de","fr","cs","uk"})for(int mask=0;mask<8;mask++) {
            ...
            Require((await File.ReadAllTextAsync(Path.Combine(root,"paws_patch_versions.ini"))).Contains("PawPatch=0.3.1-beta.1"),"Stale menu version");
        ...
        File.Copy(Path.Combine(root,".pawpatch/state.json"),Path.Combine(stage,"installed-031-beta1-state.json"));
        ...
        var report=new{passed=true,selections,uniqueHelpers=seen.Count,frameChecks,preflights=56,transitions=51,gameLaunched=false,sourceGameReadOnly=true,fixture=root};
