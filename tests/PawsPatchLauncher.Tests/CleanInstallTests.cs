...
public static class CleanInstallTests
{
    public static async Task RunAsync(string stableFeed, string betaFeed, string publicKey, string stockExe, string fixtureRoot)
    {
        ...
        foreach (var name in new[] { "stable-en", "stable-ru", "beta-en", "beta-ru" })
        {
            ...
            var executable = Path.Combine(game, GameExecutableSelector.Select(config, beta, settings.DesyncMode == "continue", settings.IndependentHostility));
        ...
        await File.WriteAllTextAsync(Path.Combine(root, "results.json"), JsonSerializer.Serialize(new { passed = true, profiles = results, gameLaunched = false, tempRoot = root }));
