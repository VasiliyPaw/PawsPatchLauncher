...
public static class Arcane030Tests
{
    ...
    public static async Task RunAsync(string repoRoot,string stageRoot,string gameRoot)
    {
        ...
        Require(feed.Channel=="stable"&&feed.PatchGuide!.Version=="0.3.0","Wrong release identity");
        ...
        http.DefaultRequestHeaders.UserAgent.ParseAdd("PawsPatchArcane030Validation/1");
        ...
        foreach(var scenario in scenarios.Values){
            ...
            if(scenario.Settings.PawPatchEnabled&&!scenario.Settings.DataOnly){
                string source=Path.Combine(scenario.Settings.Channel=="stable"?stage:Path.Combine(Path.GetDirectoryName(stage)!,"release-arcane-beta9"),"helpers",scenario.Exe);
        ...
        async Task Apply(ChannelManifest channel,UserSettings raw,bool preflight){
            ...
            if(s.PawPatchEnabled&&!s.DataOnly&&channel==feed){
                Require((await File.ReadAllTextAsync(Path.Combine(root,"paws_patch_versions.ini"))).Contains("PawPatch=0.3.0\n"),"Wrong menu identity");
        ...
        File.Copy(Path.Combine(root,".pawpatch/state.json"),Path.Combine(stage,"installed-030-state.json"));
        ...
        Require(!Directory.EnumerateFiles(Path.Combine(root,"skins"),"Background.tga",SearchOption.AllDirectories).Any(),"Frames survived old-release rollback");
