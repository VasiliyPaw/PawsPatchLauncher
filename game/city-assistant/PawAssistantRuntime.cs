...
internal static partial class PawAssistantRuntime
{
    ...
    private static readonly CityMilitiaPlanner militiaPlanner = new CityMilitiaPlanner();
    private static uint militiaRevision, militiaSerial, militiaPending, militiaEpoch;
        ...
        try
        {
            ...
            for (int i = 0; i < Sites.Length; i++)
            {
                ...
                else
                {
                    ...
                    Array.Copy(BitConverter.GetBytes(unchecked(cave+InlineEntries[i-10]-(module+Sites[i]+5))),0,replacement,1,4);
                }
                attempted = i + 1;
    ...
    internal static void Install(Process game, IntPtr module, IntPtr signal, string root, Action<string> log)
    {
        ...
        try
        {
            ...
            logger = log;
            log("ASSISTANT beta policy=r20 installed; partyPreferences=true; cityOrders=true; nativeQueue=true; mines=true; newCityAndBuildingMilitia="+policy.NewCitiesOpenMilitia+"; noticeCooldown=300000ms; nativeNotice=" + (signal != IntPtr.Zero) + ".");
        }
    ...
    internal static void Tick()
    {
        ...
        if(!settingsOpen)
        {
            ...
            if(result!=null)
            {
                ...
                result.NewCitiesOpenMilitia=policy.NewCitiesOpenMilitia;
                policy=result;
    ...
    private static void Plan(byte[] header, byte[] ui, byte[] records, byte[] cityRecords, byte[] construction, byte[] forecast)
    {
        ...
        for (int i=0;i<s.Cities.Length;i++) if ((BitConverter.ToUInt32(cityRecords,i*8+4)&1)!=0) s.Busy.Add(s.Cities[i]);
    ...
    private static void InitializeNativePolicy()
    {
        ...
        memory.Write(state+0xc0,BitConverter.GetBytes(policy.NewCitiesOpenMilitia?1:0));
    ...
    private static void ReadNativeMilitiaPreference()
    {
        uint revision=Pointer(state+0xc8);
        if(revision==militiaRevision)return;
        uint value=Pointer(state+0xc0);
        if(value>1 || revision!=Pointer(state+0xc8))return;
        militiaRevision=revision;policy.NewCitiesOpenMilitia=value==1;
        logger("ASSISTANT F1 new cities open militia="+policy.NewCitiesOpenMilitia);
        try{policy.Save(policyPath);}catch(Exception ex){logger("ASSISTANT militia preference save failed: "+ex.Message);}
    }
    private static void PlanMilitia(byte[] header,byte[] cityRecords,byte[] records)
    {
        ...
        if(militiaPending!=0)
        {
            ...
            if(BitConverter.ToUInt32(reply,4)==militiaPending)
            {
                uint result=BitConverter.ToUInt32(reply,0);
                if(epoch==militiaEpoch)militiaPlanner.Reply(result,time);
                logger("ASSISTANT MILITIA reply="+militiaPending+" result="+result);
                militiaPending=0;
            }
        }
