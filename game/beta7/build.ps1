...
$shared = 'TerrainRuntime.cs','RandomMapPatch.cs','ReleaseStartup.cs','LobbyColorsNative.cs','GamePresentation1372.cs','RandomMapBundle.cs'
...
foreach ($variant in $variants.PSObject.Properties) {
    ...
    $arguments += Join-Path $PSScriptRoot ([IO.Path]::GetFileNameWithoutExtension($variant.Name) + '.cs')
...
Write-Output 'Built six quiet helpers. No game launched. Install only with the matching signed Beta data packages.'
