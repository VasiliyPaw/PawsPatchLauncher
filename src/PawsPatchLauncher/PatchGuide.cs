...
public static class PatchGuide
{
    ...
    public static IReadOnlyList<PatchGuideEntry> LegacyEntries { get; } = [
        ...
        new("localization", "optional", "Локализация игры", "Game language",
            "Текст и озвучку можно выбрать отдельно. Оба выбора сохраняются при переключении модов и применяются вместе с настройками. Скачиваются только выбранные языки; уже загруженные файлы используются повторно, пока не выйдет их обновление.\n\nПеревод текста охватывает оригинальную игру, Immortals, Arcane Wars и Paw's Patch. Доступные языки озвучки могут отличаться от языков текста. Язык лаунчера настраивается отдельно. Локализация не меняет игровые характеристики.",
            "Choose text and speech separately. Both choices persist across mods and are applied with your settings. Only selected languages are downloaded; cached files are reused until an update is available.\n\nText translations cover the original game, Immortals, Arcane Wars and Paw's Patch. Available speech languages may differ from text languages. Launcher language is configured separately. Localization does not change gameplay statistics."),
