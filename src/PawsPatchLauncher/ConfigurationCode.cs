...
public static class ConfigurationCode
{
    public static UserSettings Parse(string code)
    {
        ...
        if (parts.Length != 10 || parts[0] != "PAW" || parts[1] is not ("BETA" or "STABLE"))
    ...
    public static string Create(UserSettings settings)
    {
        ...
        return $"PAW-{channel}-IW{Bit(settings.IndependentHostility)}-SP{spawn}-RM{Bit(settings.AdditionalRoamingCompanies)}-SG{Bit(settings.SiegeBalance)}-LM1-RU{Bit(settings.RussianLocalization)}-CL{Bit(settings.CustomPlayerColors)}-OOS{oos}";
    }
