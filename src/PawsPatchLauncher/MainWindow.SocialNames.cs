...
public partial class MainWindow
{
    private FrameworkElement SocialNameLabel(SocialPlayer player)
    {
        ...
        if(!player.Deleted&&(player.AdminLevel>0||player.PawsTeam))
        {
            username.Margin=new Thickness(0);
            var badge=PlayerRoleBadge(player.AdminLevel,player.PawsTeam)!;badge.Margin=new Thickness(0);badge.Padding=new Thickness(6,2,6,2);
